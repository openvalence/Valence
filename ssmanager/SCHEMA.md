# SSManager manifest schema (v1)

A repo declares its tools in `ssmanager.toml` at its root. SSManager reads
manifests and spawns processes; it knows nothing else. Governing rules are
SlopDrive-32 `docs/canon/DOCTRINE.md` §10 — the short form:

* Adding a tool is a manifest entry and **zero** lines of SSManager code.
...
inputs = ["lib/slopsync/**", "spec/**", "tools/slopsync_lint.py"]
needs  = ["python"]
order  = 10
```
...
toolchain SSManager has never heard of.
...
`run`, SSManager substitutes the resolved candidate. So `run = ["pio", ...]`
stays exactly what a developer types when `pio` is on PATH, and silently
works when it is not. This is the ONLY substitution SSManager performs;
