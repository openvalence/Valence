...
// lib/valence/.../generated/registry_constants.hpp). The byte-level behavior
// mirrors tools/valence_probe.py, the live-verified Python reference client —
// where the spec and the probe disagree, the probe wins.
//
// v0.3.0 — Valence v1.0 wire. Three things changed shape:
...
public class Valence : PluginBase
{
    ...
    {
        var b = new byte[ValenceWire.InstanceIdBytes];
    ...
    private async Task SessionAsync(CancellationToken token)
    {
        ...
        // §5.5.3) but it is pure transport-layer duplication here: Valence
        ...
        if (mode == StreamMode.Segments)
        {
            ...
            segGrantedBurst = welcome.GrantedPublishBurst(ValenceWire.ChMotionSegment);
        ...
        finally
        {
            ...
            try
            {
                ...
                await client.GoodbyeAsync(ValenceWire.GoodbyeNormalClosure, byeCts.Token);
            }
    ...
    private void OnNack(HubClient.NackInfo n)
    {
        ...
        {
            NackCount++;
            if (n.Code == ValenceWire.NackRateLimited) RateLimitedCount++;
    ...
    private ValenceCatalog _catalog;
    private ValenceCatalog.RoleLocator _roleWindowMin, _roleWindowMax;
    ...
    private static CachedCatalog LoadCachedCatalog(string key)
    {
        lock (_catalogCacheLock)
        {
            ...
            if (ValenceCatalog.BytesEqual(ValenceCatalog.Etag(c.Bytes), c.Etag)) return c;
    ...
    private async Task EstablishReadiness(HubClient client, WelcomeInfo welcome, CancellationToken token)
    {
        ...
        if (hubEtag == null || hubEtag.Length != ValenceWire.EtagBytes)
            throw new InvalidOperationException(
                $"WELCOME carried no usable {ValenceWire.EtagBytes}-byte catalog_etag — cannot declare readiness");
        ...
        if (_cachedForThisSession != null && ValenceCatalog.BytesEqual(_cachedForThisSession.Etag, hubEtag))
        {
        ...
        var digest = ValenceCatalog.Etag(bytes);
        bool verified = ValenceCatalog.BytesEqual(digest, hubEtag);
        if (!verified)
        {
            ...
            Logger.Warn("catalog reassembled to etag {0} but WELCOME advertised {1} — declaring what we hold",
                ValenceCatalog.Hex(digest), ValenceCatalog.Hex(hubEtag));
        }

        var cat = ValenceCatalog.Decode(bytes);
        if (cat == null)
        ...
        Logger.Info("CATALOG_READY sent (etag {0}, {1} B, {2} channels{3})",
            ValenceCatalog.Hex(digest), bytes.Length, cat.Entries.Count, verified ? "" : ", UNVERIFIED");
    ...
    private void AdoptCatalog(ValenceCatalog cat, byte[] etag, bool cachedPath)
    {
        _catalog = cat;
        _roleWindowMin = cat.LocateRole(ValenceWire.RoleWindowMin);
        _roleWindowMax = cat.LocateRole(ValenceWire.RoleWindowMax);
        _roleInputSpeed = cat.LocateRole(ValenceWire.RoleLimitInputSpeed);
        _roleInputAccel = cat.LocateRole(ValenceWire.RoleLimitInputAccel);
        _roleInputJerk = cat.LocateRole(ValenceWire.RoleLimitInputJerk);
        _rolePosition = cat.LocateRole(ValenceWire.RoleTelemetryPosition);
        _roleTarget = cat.LocateRole(ValenceWire.RoleTelemetryTarget);
        ...
        static string Describe(ValenceCatalog.RoleLocator r) =>
    ...
    private void AdoptRoleReadback(ushort channel, byte[] payload)
    {
        ...
        double? Read(ValenceCatalog.RoleLocator r)
        {
            ...
            double v = ValenceCatalog.ReadField(payload, r.Field);
    ...
    private async Task PumpCatalogReadyAsync(HubClient client, double nowMs, CancellationToken token)
    {
        ...
        if (nowMs - _readyLastSendMs < ValenceWire.CatalogChunkGapTimeoutMs) return;
    ...
    private async Task StreamLoopAsync(HubClient client, double rateHz, CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (nowMs - lastResyncMs >= ValenceWire.ClockResyncIntervalMs)
            {
                lastResyncMs = nowMs;
                await ResyncClock(client, token);
            }
    ...
    // span's mean velocity, no monotone quintic exists — VMotion's legality
...
// =============================================================================
// ValenceWire — registry constants (spec/registry/registry.yaml is the
// single source of truth) + the CBOR / frame / STREAM codec. Every number here
// is copied from the registry and cross-checked against tools/valence_probe.py.
// =============================================================================
public static class ValenceWire
{
    public const byte ProtocolVersion = 1;                 // registry proto_ver
    public const string WsSubprotocol = "valence.v1";     // limits.ws_subprotocol
    ...
    // sample scales (ValenceCatalog 0x2100): target *10000, vel *1000.
    ...
    // Scales (ValenceCatalog 0x2101): target *10000, duration *1, end_vel *1000.
...
// map keys written ascending by the caller. Mirrors valence_probe.py's encoder.
// =============================================================================
...
public sealed class WelcomeInfo
{
    ...
    public static WelcomeInfo Parse(byte[] payload)
    {
        ...
        if (map.TryGetValue(ValenceWire.KSessionId, out var sid)) w.SessionId = Convert.ToInt64(sid);
        if (map.TryGetValue(ValenceWire.KBootId, out var bid)) w.BootId = (uint)Convert.ToInt64(bid);
        if (map.TryGetValue(ValenceWire.KCatalogEtag, out var et) && et is byte[] etb) w.CatalogEtag = etb;
        if (map.TryGetValue(ValenceWire.KCfgGen, out var cg)) w.CfgGen = Convert.ToInt64(cg);
        if (map.TryGetValue(ValenceWire.KRoles, out var rl)) w.Roles = Convert.ToInt64(rl);
        if (map.TryGetValue(ValenceWire.KDeadmanMs, out var dm)) w.DeadmanMs = Convert.ToInt64(dm);
        if (map.TryGetValue(ValenceWire.KDeadmanPolicy, out var dp)) w.DeadmanPolicy = Convert.ToInt64(dp);
        if (map.TryGetValue(ValenceWire.KGrantedPublishes, out var gp) && gp is List<object> list)
        {
...
// =============================================================================
// ValenceCatalog — the hub's self-describing channel list (§8.1), decoded from the
...
public sealed class ValenceCatalog
{
    ...
    public static ValenceCatalog Decode(byte[] bytes)
    {
        ...
        var cat = new ValenceCatalog();
        foreach (var item in arr)
        {
            if (item is not Dictionary<long, object> m) continue;
            if (!m.TryGetValue(ValenceWire.CatEId, out var idv)) continue;

            var e = new Entry { Id = (ushort)Convert.ToInt64(idv) };
            if (m.TryGetValue(ValenceWire.CatEName, out var nm)) e.Name = nm as string;
            if (m.TryGetValue(ValenceWire.CatECls, out var cl)) e.Cls = (int)Convert.ToInt64(cl);
            if (m.TryGetValue(ValenceWire.CatESettingChannel, out var sc))
            ...
            if (m.TryGetValue(ValenceWire.CatELayout, out var lay) && lay is List<object> fields)
            {
                int off = 0;
                foreach (var fo in fields)
                {
                    ...
                    if (fm.TryGetValue(ValenceWire.CatFName, out var fn)) f.Name = fn as string;
                    if (fm.TryGetValue(ValenceWire.CatFType, out var ft)) f.Type = (int)Convert.ToInt64(ft);
                    if (fm.TryGetValue(ValenceWire.CatFUnit, out var fu)) f.Unit = fu as string;
                    if (fm.TryGetValue(ValenceWire.CatFScale, out var fs)) f.Scale = Convert.ToDouble(fs);
                    if (fm.TryGetValue(ValenceWire.CatFRole, out var fr)) f.Role = fr as string;
                    if (fm.TryGetValue(ValenceWire.CatFSettingKey, out var sk)) f.SettingKey = (int)Convert.ToInt64(sk);
                    if (fm.TryGetValue(ValenceWire.CatFMin, out var mn)) f.Min = Convert.ToDouble(mn);
                    if (fm.TryGetValue(ValenceWire.CatFMax, out var mx)) f.Max = Convert.ToDouble(mx);
                    if (fm.TryGetValue(ValenceWire.CatFStep, out var st)) f.Step = Convert.ToDouble(st);
                    if (fm.TryGetValue(ValenceWire.CatFGroup, out var gp)) f.Group = gp as string;
                    if (fm.TryGetValue(ValenceWire.CatFDesc, out var ds)) f.Desc = ds as string;
                    if (f.Scale == 0 || double.IsNaN(f.Scale)) f.Scale = 1.0;

                    if (f.Type < 0 || f.Type >= ValenceWire.PackedSize.Length) break;
                    f.Size = ValenceWire.PackedSize[f.Type];
                    off += f.Size;
                    e.Layout.Add(f);
                }
            }
            ...
            if (m.TryGetValue(ValenceWire.CatESchema, out var sch) && sch is Dictionary<long, object> sm)
            {
    ...
    public static double ReadField(byte[] payload, Field f)
    {
        ...
        switch (f.Type)
        {
            case ValenceWire.PackedU8:
            case ValenceWire.PackedBitfield8: raw = s[0]; break;
            case ValenceWire.PackedI8: raw = (sbyte)s[0]; break;
            case ValenceWire.PackedU16: raw = BinaryPrimitives.ReadUInt16LittleEndian(s); break;
            case ValenceWire.PackedI16: raw = BinaryPrimitives.ReadInt16LittleEndian(s); break;
            case ValenceWire.PackedU32: raw = BinaryPrimitives.ReadUInt32LittleEndian(s); break;
            case ValenceWire.PackedI32: raw = BinaryPrimitives.ReadInt32LittleEndian(s); break;
            case ValenceWire.PackedF32: raw = BinaryPrimitives.ReadSingleLittleEndian(s); break;
            default: return double.NaN;      // strings have no numeric reading
        }
        // A bitfield8 is a raw byte; scale is meaningless on it.
        if (f.Type == ValenceWire.PackedBitfield8) return raw;
    ...
    public static byte[] Etag(byte[] bytes)
    {
        ...
        var e = new byte[ValenceWire.EtagBytes];
        Array.Copy(d, e, ValenceWire.EtagBytes);
        return e;
    }
...
public sealed class BlobReassembler
{
    ...
    public bool Insert(in ValenceWire.BlobChunk c)
    {
        if (c.TotalBytes > ValenceWire.CatalogMaxBytes || c.ChunkCount == 0) return false;
...
public sealed class HubClient
{
    ...
    public sealed class NackInfo
    {
        ...
        public string Name => ValenceWire.NackName(Code);
    }
    ...
    {
        var frame = ValenceWire.EncodeFrame(type, channel, payload, seq);
    ...
    public async Task<WelcomeInfo> HelloAsync(string kind, string name,
        ...
        byte[] cachedEtag = null)
    {
        var hello = ValenceWire.BuildHello(kind, name, _instanceId, publishes, token16, subscribes, cachedEtag);
        await SendFrameAsync(ValenceWire.FHello, 0, hello, 0, token);
        ...
        while (true)
        {
            ...
            if (hdr.Type == ValenceWire.FWelcome)
                return WelcomeInfo.Parse(payload);
            if (hdr.Type == ValenceWire.FNack)
            {
                var m = (Dictionary<long, object>)new CborReader(payload).Decode();
                ushort code = m.TryGetValue(ValenceWire.KCode, out var c) ? (ushort)Convert.ToInt64(c) : (ushort)0;
                throw new InvalidOperationException($"HELLO refused: NACK {ValenceWire.NackName(code)}");
            }
    ...
    {
        var payload = ValenceWire.BuildSubscribe(wishes);
        await SendFrameAsync(ValenceWire.FSubscribe, 0, payload, 0, token);
    }
    ...
    public Task SendCatalogReadyAsync(byte[] etag, CancellationToken token)
        => SendFrameAsync(ValenceWire.FCatalogReady, 0, etag, 0, token);
    ...
    public async Task<byte[]> FetchCatalogAsync(CancellationToken token)
    {
        ...
        await SendFrameAsync(ValenceWire.FBlobReq, 0, ValenceWire.BuildCatalogRequest(), 0, token);
        ...
        using var cts = CancellationTokenSource.CreateLinkedTokenSource(token);
        cts.CancelAfter(TimeSpan.FromMilliseconds(ValenceWire.CatalogReadyTimeoutMs));
        ...
        while (!cts.IsCancellationRequested)
        {
            ...
            if (blob.Active && !blob.Complete &&
                nowMs - lastChunkMs >= ValenceWire.CatalogChunkGapTimeoutMs &&
                nowMs - lastRepairMs >= ValenceWire.CatalogChunkGapTimeoutMs)
            {
                var missing = blob.MissingIndices();
                if (missing.Count > 0 && repairs < 8)
                {
                    if (missing.Count > 32) missing = missing.GetRange(0, 32);
                    await SendFrameAsync(ValenceWire.FBlobReq, 0, ValenceWire.BuildCatalogRepair(missing), 0, token);
            ...
            ValenceWire.FrameHeader hdr;
            byte[] payload;
            try { (hdr, payload) = await ReceiveFrameAsync(cts.Token); }
            catch (OperationCanceledException) { break; }

            if (hdr.Type != ValenceWire.FBlobChunk)
            {
                ...
                if (hdr.Type == ValenceWire.FNack)
                {
                    var m = (Dictionary<long, object>)new CborReader(payload).Decode();
                    ushort code = m.TryGetValue(ValenceWire.KCode, out var c) ? (ushort)Convert.ToInt64(c) : (ushort)0;
                    _log.Warn("catalog transfer NACK {0}", ValenceWire.NackName(code));
                    if (code == ValenceWire.NackChunkUnavailable) return null;
                }
                continue;
            }

            if (!ValenceWire.TryDecodeBlobChunk(payload, out var chunk))
            {
                _log.Warn("BLOB_CHUNK shorter than the {0}-byte identity header", ValenceWire.BlobChunkHeaderBytes);
                continue;
            }
            if (chunk.Ns != ValenceWire.BlobNsCatalog) continue;   // not our namespace
    ...
    {
        var payload = ValenceWire.BuildIntent(channelId, intentId, value);
        return SendFrameAsync(ValenceWire.FIntent, channelId, payload, (ushort)(intentId & 0xFFFF), token);
    }
    ...
    public Task GoodbyeAsync(ushort code, CancellationToken token) =>
        SendFrameAsync(ValenceWire.FGoodbye, 0, ValenceWire.BuildGoodbye(code), 0, token);
    ...
    public async Task<(long offset, long rtt)?> ClockExchangeAsync(CancellationToken token)
    {
        ...
        uint t0 = ClientNowUs();
        await SendFrameAsync(ValenceWire.FClock, 0, ValenceWire.BuildClockRequest(t0), 0, token);
        ...
        try
        {
            ...
            reg.Dispose();
            long offset = (ValenceWire.WrapDiff(t1, t0e) + ValenceWire.WrapDiff(t2, t3)) / 2;
            long rtt = ValenceWire.WrapDiff(t3, t0e) - ValenceWire.WrapDiff(t2, t1);
            return (offset, rtt);
        }
    ...
    {
        var payload = ValenceWire.BuildStreamBundle(tBase, new (ushort, double, double)[] { (0, target, vel) });
        ushort seq = _streamSeq++;
        return SendFrameAsync(ValenceWire.FStream, ValenceWire.ChMotionInput, payload, seq, token);
    }
    ...
    private void Dispatch(ValenceWire.FrameHeader hdr, byte[] payload, CancellationToken token)
    {
        switch (hdr.Type)
        {
            case ValenceWire.FClock:
                ...
                break;
            case ValenceWire.FPing:
                // §6.5: answer PING with PONG echoing the payload.
                _ = SendFrameAsync(ValenceWire.FPong, hdr.Channel, payload, 0, token);
                break;
            case ValenceWire.FNack:
            {
                var m = (Dictionary<long, object>)new CborReader(payload).Decode();
                var info = new NackInfo
                {
                    Code = m.TryGetValue(ValenceWire.KCode, out var c) ? (ushort)Convert.ToInt64(c) : (ushort)0,
                    Channel = m.TryGetValue(ValenceWire.KChannelId, out var cc) ? (ushort)Convert.ToInt64(cc) : hdr.Channel,
                    Detail = m.TryGetValue(ValenceWire.KDetail, out var d) ? d as string : null,
                    ...
                    IntentSeq = m.TryGetValue(ValenceWire.KIntentSeq, out var s) ? Convert.ToInt64(s) : (long?)null,
                };
                _onNack?.Invoke(info);
                break;
            }
            case ValenceWire.FState:
            ...
            case ValenceWire.FEcho:
            {
                ...
                {
                    Channel = m.TryGetValue(ValenceWire.KChannelId, out var cc) ? (ushort)Convert.ToInt64(cc) : hdr.Channel,
                    IntentId = m.TryGetValue(ValenceWire.KIntentId, out var ii) ? Convert.ToInt64(ii) : -1,
                    CfgGen = m.TryGetValue(ValenceWire.KCfgGen, out var cg) ? Convert.ToInt64(cg) : -1,
                    Applied = m.TryGetValue(ValenceWire.KApplied, out var ap) ? ap as Dictionary<long, object> : null,
                };
                _onEcho?.Invoke(e);
                break;
            }
            case ValenceWire.FEvent:
                ...
                {
                    var m = (Dictionary<long, object>)new CborReader(payload).Decode();
                    long kind = m.TryGetValue(ValenceWire.KEventKind, out var k) ? Convert.ToInt64(k) : -1;
                    bool hasBody = m.ContainsKey(ValenceWire.KBody);
                ...
                break;
            case ValenceWire.FGoodbye:
            {
                ushort code = 0;
                try
                {
                    var m = (Dictionary<long, object>)new CborReader(payload).Decode();
                    if (m.TryGetValue(ValenceWire.KCode, out var c)) code = (ushort)Convert.ToInt64(c);
                }
                catch { /* an empty GOODBYE is legal */ }
                _log.Info("hub sent GOODBYE: {0}", ValenceWire.NackName(code));
                break;
            }
            case ValenceWire.FBlobChunk:
                ...
                break;
            case ValenceWire.FGrant:
            case ValenceWire.FWelcome:
            case ValenceWire.FPong:
            case ValenceWire.FBeacon:
            case ValenceWire.FPairGrant:
            case ValenceWire.FHubSig:
            case ValenceWire.FPublish:
            default:
    ...
    // Reads exactly one Valence frame (one binary WS message). The firmware
    ...
    private async Task<(ValenceWire.FrameHeader, byte[])> ReceiveFrameAsync(CancellationToken token)
    {
        ...
        var data = ms.ToArray();
        if (!ValenceWire.TryDecodeHeader(data, out var hdr))
            throw new FormatException($"frame shorter than {ValenceWire.HeaderBytes}-byte header ({data.Length} B)");
        int len = Math.Min(hdr.Len, Math.Max(0, data.Length - ValenceWire.HeaderBytes));
        var payload = new byte[len];
        Array.Copy(data, ValenceWire.HeaderBytes, payload, 0, len);
        return (hdr, payload);
    }
}
