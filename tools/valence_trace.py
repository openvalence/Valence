...
WHAT IT IS FOR
    ...
    lines on one mm axis:
        ...
        PLANNED   0x1100 tgt_10um   -- where VMotion is driving to
...
SAFETY GUARANTEE
    Valence Trace connects as a `watch`-tier session and SUBSCRIBES ONLY. It never
...
RELATIONSHIP TO tools/valence_probe.py
    The probe is this project's primary verification client and stays
    untouched. Valence Trace IMPORTS it as a wire library -- CBOR codec, frame
    ...
    Valence wire in tools/ and it is the heavily-exercised one. What
    Valence Trace adds on top is a catalog-DRIVEN decoder: where the probe
    hardcodes struct layouts (correct for a conformance tool that must fail
    when the device drifts), Valence Trace decodes every packed STATE payload from
...
USAGE
    python valence_trace.py record  [--ip IP] [--port 82] [--seconds S] [--out T.jsonl]
                                [--render G.html]
    python valence_trace.py live    [--ip IP] [--port 82] [--seconds S] [--out T.jsonl]
    python valence_trace.py render  T.jsonl [--out G.html] [--theme dark|light]
...
import valence_probe as ss  # noqa: E402


TOOL_NAME = "valence_trace"
TOOL_VERSION = "1.0.0"
...
class Resolved:
    """Everything Valence Trace needs, plus a written record of HOW each was
...
class ScopeSession:
    ...
    def hello(self):
        ...
        policy grants; Valence Trace only ever needs `watch`."""
    ...
    def keepalive(self):
        ...
        stopping.) So Valence Trace pings on ITS OWN clock, well inside the
...
def http_capabilities(ip, http_port, timeout=2.0):
    ...
    perfectly good Valence hub; the trace just records null."""
    try:
    ...
    return {
        ...
        "firmware": {
            ...
            "device": ((caps or {}).get("device") or (caps or {}).get("name")
                       or ("valencesim (SIMULATED MACHINE)" if (caps or {}).get("sim") else None)
        ...
        "safety": ("Valence Trace subscribes only: watch tier, no publishes wish, "
...
def capture(args, on_sample=None):
    ...
    url = sess.connect()
    print("[valence_trace] connected %s (subprotocol %s)" % (url, ss.WS_SUBPROTOCOL))
    try:
        sess.hello()
        role = sess.welcome.get(ss.K["roles"])
        print("[valence_trace] WELCOME session=%s roles=%s (watch tier is all this tool needs)"
              % (sess.welcome.get(ss.K["session_id"]), role))
        verified, digest = sess.fetch_catalog()
        print("[valence_trace] catalog %d B, %d entries, etag %s"
        ...
        for line in ("  resolve %-11s %s" % (k, v) for k, v in sess.res.how.items()):
            print(line)
        if miss:
            raise RuntimeError("this hub does not expose what Valence Trace plots: "
                               + "; ".join(miss))

        grants = sess.subscribe()
        print("[valence_trace] grants: %s" % ", ".join(
...
def run_live(args):
    ...
    def paint(kind, rec, state):
        ...
        L.append("Valence Trace live  ·  %s:%d  ·  fw %s  ·  watch tier, subscribe-only" % (
    ...
    print("\n[valence_trace] %s" % _summary_line(header, state))
    return 0
    ...
    with io.open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if o.get("rec") == "header":
                ...
                recs.append(o)
    if header is None:
        raise RuntimeError("%s has no header record -- not a Valence Trace trace" % path)
    return header, recs
...
def _title(header):
    ...
    return "Valence Trace %s %s" % (header.get("started_utc", ""), ("fw %s" % fw) if fw else "")
...
def add_conn_args(p):
    ...
    p.add_argument("--port", type=int, default=82, help="Valence WS port (default 82)")
...
def main():
    ap = argparse.ArgumentParser(
        ...
        description=(
            "Valence Trace -- graph what Valence was ASKED for against what the motion planner\n"
            "PLANNED and what the machine ACHIEVED, in mm inside the stroke window.\n"
            "\n"
            "SAFETY: Valence Trace connects as a `watch`-tier session and SUBSCRIBES ONLY.\n"
        ...
        epilog=(
            "examples:\n"
            "  valence_trace.py record --ip 127.0.0.1 --seconds 20 --out run.jsonl --render run.html\n"
            "  valence_trace.py live   --ip 192.168.1.229 --seconds 60\n"
            "  valence_trace.py render run.jsonl --out run.html --theme light\n"))
    ...
    p.add_argument("--out", default=None, help="trace file (default valence_trace-<stamp>.jsonl)")
    ...
    if args.cmd == "render":
        ...
        print("[valence_trace] %s  (%d motion, %d plan, %d segments, %d anomalies)"
    ...
    if args.cmd == "live":
        ...
        try:
            ...
            print("[valence_trace] %s" % e)
            return 1

    # record
    if not args.out:
        args.out = "valence_trace-%s.jsonl" % time.strftime("%Y%m%d-%H%M%S")
    try:
        header, state = capture(args)
    except RuntimeError as e:
        print("[valence_trace] %s" % e)
        return 1
    print("[valence_trace] %s -> %s" % (_summary_line(header, state), args.out))
    if args.render:
...
<div class="wrap">
<h1>Valence Trace — asked · planned · achieved</h1>
